...
import 'package:hackaton_front/Models/prociscivac.dart';
...
class ProciscivacPage extends StatefulWidget {
  ...
  @override
  State<ProciscivacPage> createState() => _ProciscivacPageState(selectedIndex, objekat);
}
                              ...
                              children: [
                                const Icon(Icons.air, size: 150, color: Colors.white),
                                    ...
                                    Padding(
                                      ...
                                      child: Text(objekat == null ? "Choose air refreshener": objekat!.naziv,
                                ...
                                Padding(
                                          padding: const EdgeInsets.all(30),
                                          child: Text(objekat == null ? "First choose device" : objekat!.stanje == true ? "Device is ON" : "Device is OFF",
                                          style: const TextStyle(color: Colors.white, fontSize: 15)),
              ...
              onTap: () {
                          ...
                          objekat = snapshot.data![index];
                            ...
                            Navigator.of(context).pushReplacement(
                                      MaterialPageRoute(
                                        builder: (context) => ProciscivacPage(selectedIndex, objekat),
                                      ),
                                    );
                          });
                        },
...
addDevice(nazivController) async {
  var data = RoleUtil.getData();
  final response = await http.post(Uri.parse('${GlobalUrl.url}ProciscivacZraka/Snimi'),
...
Future<List<Prociscivac>> fetch() async{
  var data = RoleUtil.getData();
  var url = Uri.parse('${GlobalUrl.url}ProciscivacZraka/GetByHouse?homeId=${data["kucaId"]}');
